...
/** @author Jab */
public class MapRendererAlphaSpecialMask extends MapRenderer {
  ...
  public BufferedImage render(Lvl lvl, int x1, int y1, int x2, int y2) {
    ...
    for (int y = y1; y <= y2; y++) {
      for (int x = x1; x <= x2; x++) {
        int tile = mask.getTile(x, y);
        if (!Tileset.isException(tile)) {
          BufferedImage tileImage = mask.getTileset().getImage(tile);
          if (tileImage != null) {
