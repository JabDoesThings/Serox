...
public class ByteArray {
  ...
  public int readLittleEndianShort(int index) {
    return (int) (((m_array[index + 1] & 0xff) << 8) | ((m_array[index] & 0xff)));
  }

  public int readInt(int index) {
    return (int)
        (((m_array[index] & 0xff) << 24)
            | ((m_array[index + 1] & 0xff) << 16)
            | ((m_array[index + 2] & 0xff) << 8)
            | ((m_array[index + 3] & 0xff)));
  }

  public int readLittleEndianInt(int index) {
    return (int)
        (((m_array[index + 3] & 0xff) << 24)
            | ((m_array[index + 2] & 0xff) << 16)
            | ((m_array[index + 1] & 0xff) << 8)
            | ((m_array[index] & 0xff)));
  }

  public long readLong(int index) {
    return (long)
        (((m_array[index] & 0xff) << 56)
            | ((m_array[index + 1] & 0xff) << 48)
            | ((m_array[index + 2] & 0xff) << 40)
            | ((m_array[index + 3] & 0xff) << 32)
            | ((m_array[index + 4] & 0xff) << 24)
            | ((m_array[index + 5] & 0xff) << 16)
            | ((m_array[index + 6] & 0xff) << 8)
            | ((m_array[index + 7] & 0xff)));
  }

  public long readLittleEndianLong(int index) {
    return (long)
        (((m_array[index + 7] & 0xff) << 56)
            | ((m_array[index + 6] & 0xff) << 48)
            | ((m_array[index + 5] & 0xff) << 40)
            | ((m_array[index + 4] & 0xff) << 32)
            | ((m_array[index + 3] & 0xff) << 24)
            | ((m_array[index + 2] & 0xff) << 16)
            | ((m_array[index + 1] & 0xff) << 8)
            | ((m_array[index] & 0xff)));
